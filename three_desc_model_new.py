...
def make_symmetry_functions(turbines, type, etas, offsets=None,
    ...
    if type == 'G2':
        ...
        for eta in etas:
            for zeta in zetas:
                for gamma in gammas:
                    G.append({'type': 'G4', 'turbine':turbines, 'eta': eta, 'gamma': gamma, 'zeta': zeta})
        return G
    elif type == 'G6':
        G = []
        for eta in etas:
            for zeta in zetas:
                for gamma in gammas:
                    G.append({'type': 'G6', 'turbine':turbines, 'eta': eta, 'gamma': gamma, 'zeta': zeta})
        return G
